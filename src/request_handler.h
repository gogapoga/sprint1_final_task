...
namespace http_handler {
    ...
    public:
        ...
        void operator()(http::request<Body, http::basic_fields<Allocator>> &&req, Send &&send) {
            ...
            if ((method == http::verb::get) ||
                ...
                std::vector<std::string_view> param = Split(req.base().target(), ConstLiteralName::TEXT_SLASH);
                if (param.empty()) {
                    send(text_response(static_cast<http::status>(404), ConstLiteralName::TEXT_EMPTY));
                    return;
                }
                if (!(param.at(0) == ConstLiteralName::TEXT_API)) {
                    send(text_response(static_cast<http::status>(404), ConstLiteralName::TEXT_EMPTY));
                    return;
                }
                if ((param.size() < 3) ||
                    (param.size() > 4) ||
                    !(param.at(1) == ConstLiteralName::TEXT_V1) ||
                    !(param.at(2) == model::ConstJsonKey::TEXT_MAPS)) {
                    boost::json::object json_bad_request;
                    json_bad_request[model::ConstJsonKey::TEXT_CODE] = ConstLiteralName::TEXT_BAD_REQUEST_WITHOUT_SPACES;
                    json_bad_request[model::ConstJsonKey::TEXT_MESSAGE] = ConstLiteralName::TEXT_BAD_REQUEST_WITH_SPACES;
                ...
                if (param.size() == 4) {
                    ...
                    if (body.empty()) {
                        ...
                        json_map_not_found[model::ConstJsonKey::TEXT_CODE] = ConstLiteralName::TEXT_MAP_NOT_FOUND_WITHOUT_SPACES;
                        json_map_not_found[model::ConstJsonKey::TEXT_MESSAGE] = ConstLiteralName::TEXT_MAP_NOT_FOUND_WITH_SPACES;
                        std::ostringstream oss;
                ...
                send(text_response(http::status::method_not_allowed, ConstLiteralName::TEXT_INVALID_METHOD));
            }
        }
    ...
    private:
        ...
        MakeStringResponse(http::status status, std::string_view body, unsigned http_version, bool keep_alive,
        ...
        struct ConstLiteralName {
            ConstLiteralName() = delete;

            constexpr static std::string_view TEXT_EMPTY = ""sv;
            constexpr static std::string_view TEXT_API = "api"sv;
            constexpr static std::string_view TEXT_V1 = "v1"sv;
            constexpr static std::string_view TEXT_INVALID_METHOD = "Invalid method"sv;
            constexpr static std::string TEXT_MAP_NOT_FOUND_WITH_SPACES = "Map not found";
            constexpr static std::string TEXT_MAP_NOT_FOUND_WITHOUT_SPACES = "mapNotFound";
            constexpr static std::string TEXT_BAD_REQUEST_WITH_SPACES = "Bad request";
            constexpr static std::string TEXT_BAD_REQUEST_WITHOUT_SPACES = "badRequest";
            constexpr static char TEXT_SLASH = '/';
        };
        ...
        boost::json::array GetMapsJson() {
            ...
            for (const auto &map: vector_maps) {
                ...
                json_map[model::ConstJsonKey::TEXT_ID] = *map.GetId();
